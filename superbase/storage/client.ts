...
export const uploadImage = async ({ file, bucket, folder }: UploadProps) => {
  ...
  const HARDCODE_DOMAIN = "https://ejybbrskfalzqrtrpzma.supabase.co"; // FIX: Replace with env var

  const imageUrl = `${HARDCODE_DOMAIN}/storage/v1/object/public/${bucket}/${data?.path}`;
